...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with 
      | Empty -> false
      | Interval (low, high) -> if compare x low >= 0 && 
                                      compare x high <= 0 then true else false 
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Interval (l1, h1), Interval (l2, h2) -> 
        let newLow = if compare l1 l2 >= 0 then l1 else l2 in
        let newHigh = if compare h1 h2 <= 0 then h1 else h2 in
          if compare newLow newHigh < 0 then Interval (newLow, newHigh) else Empty 
end 
;;
...
let intvl1 = IntInterval.create 3 10 ;;
